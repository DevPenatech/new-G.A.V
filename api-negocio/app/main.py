...
def endpoint_buscar_produtos(query: esquemas.BuscaQuery, db: Session = Depends(get_db)):
    resultado_busca = crud.buscar_produtos(
        ...
        codfilial=query.codfilial,
        apenas_ofertas=query.apenas_ofertas  # NOVO PARÂMETRO
    )
...
def endpoint_buscar_contexto(
    sessao_id: str,
    tipo_contexto: Optional[str] = None,            # <-- NOVO parâmetro opcional (query param)
    limite: Optional[str] = None,  
    db: Session = Depends(get_db),
):
    """Busca o contexto mais recente de uma sessão (pode filtrar por tipo_contexto)."""
    if tipo_contexto:
        contexto = crud.buscar_contexto_sessao(db, sessao_id=sessao_id, tipo_contexto=tipo_contexto, limite=limite)
    else:
